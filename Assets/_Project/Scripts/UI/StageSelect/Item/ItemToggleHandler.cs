...
namespace CocoDoogy.StageSelect.Item
{
    public class ItemToggleHandler : MonoBehaviour
    {
        ...
        //        3. 실제로 사용되는 시점은 InGame 스테이지로 들어갔을 때 적용되도록 (Clear)
        //        4. 지금 StageReadyUI에서는 isEquipped라는 bool배열로 아이템 사용을 관리하는거 같은데 이거 지우고 여기서 Toggle의 isOn을 확인해서 반환하면 될 듯함. (Clear)
